...
std_frac = 0 # In the random initialization, how to move away from the std
...
for std_frac in std_fracs:
    for learning_rate_fine_tune in learning_rates_fine_tune:
        for initialization in initializations:
            for num_finetune_epochs in nums_finetune_epochs:
                if initialization == 0 and std_frac!=0:
                    break
                
                data_path = './data/'+dataset+'/'
                experiment_name = './'+dataset+'_'+model_name+'_fine_tune_'+str(num_finetune_epochs)+'_'+str(learning_rate_fine_tune)+'_'+str(num_negs_per_pos)+'_'+str(early_stopping_finetune)+'_'+str(pruned_dict)+'_init'+str(initialization)+'_'+str(std_frac)+'/'
                ...
                for s in range(number_snapshots):
                    ...
                    if s == 0:
                        ...
                        if reuse_snapshot_0:
                            ...
                            file_path = reuse_path+'training_triples/entity_to_id.tsv.gz'
                            df = pd.read_csv(file_path, sep='\t', compression='gzip', header=0)
                            old_ent_to_id = dict(zip(df.iloc[:, 1], df.iloc[:, 0]))

                            file_path = reuse_path+'training_triples/relation_to_id.tsv.gz'
                            df = pd.read_csv(file_path, sep='\t', compression='gzip', header=0)
                            old_rel_to_id = dict(zip(df.iloc[:, 1], df.iloc[:, 0]))

                            model = torch.load(reuse_path+'trained_model.pkl') #WARNING: In my laptop maybe delete the map_location=torch.device('cpu')

                            # Get the embeddings
                            old_ent_emb = model.entity_representations[0](indices= None).clone().detach().to(device)
                            old_rel_emb = model.relation_representations[0](indices= None).clone().detach().to(device)
                            old_rel_norm_emb = model.relation_representations[1](indices= None).clone().detach().to(device)

                            src = reuse_path+'results.json'
                            dst = experiment_name+'0/results.json'
                            shutil.copy(src, dst)
                            ...
                            train = pipeline(
                                ...
                                model='transr',
                                model_kwargs= {'embedding_dim': emb_dimension,'relation_dim': emb_dimension},
                        ...
                        model = TransR(triples_factory=training_triples_factory, embedding_dim=emb_dimension, relation_dim=emb_dimension, random_seed = 8128).to(device)
                        ...
                        with torch.no_grad():
                            ...
                            model.relation_representations[1]._embeddings.weight.copy_(new_rel_norm_emb.view(model.num_relations,emb_dimension*emb_dimension))
                        ...
                        if initialization == 1: # Initialize new entities with the average of their classes:
                            ...
                            new_ent_emb = init_class_avg(old_ent_to_id,old_ent_emb,new_ent_to_id,new_ent_emb,emb_dimension,pruned_dict,random_noise, std_frac).to(device)
                            with torch.no_grad():
                                model.entity_representations[0]._embeddings.weight.copy_(new_ent_emb)

                        elif initialization == 2: # Initialize with weighted average of classes, inversly proportional to the size of the class

                            new_ent_emb = init_weighted_class_avg(old_ent_to_id,old_ent_emb,new_ent_to_id,new_ent_emb,emb_dimension,pruned_dict,random_noise, std_frac).to(device)
                            with torch.no_grad():
                                model.entity_representations[0]._embeddings.weight.copy_(new_ent_emb)

                        elif initialization == 3: #Initialize with triples results
                            
                            new_ent_emb = init_on_triples(s,old_ent_to_id,old_ent_emb,old_rel_to_id,old_rel_emb,new_ent_to_id,new_ent_emb,emb_dimension,data_path,random_noise,std_frac).to(device)
                            with torch.no_grad():
                                model.entity_representations[0]._embeddings.weight.copy_(new_ent_emb)
                        ...
                        del validation_triples
                        torch.cuda.empty_cache()
                        #torch.save(model.state_dict(), experiment_name+str(s)+'/model.pt')  #We are not saving the model anymore
                        ...
                        del model, training_loop, evaluator, results, new_ent_emb, new_rel_emb, new_rel_norm_emb
                        torch.cuda.empty_cache()
                
                del old_ent_emb, old_rel_emb
                torch.cuda.empty_cache()
                gc.collect()
