            ...
            for num_finetune_epochs in nums_finetune_epochs:
                ...
                for s in range(number_snapshots):
                    ...
                    if s == 0:
                        ...
                        old_rel_emb_norm = train.model.relation_representations[1](indices= None).clone().detach().to(device)
                        ...
                        model = SimplE(triples_factory=training_triples_factory, embedding_dim=emb_dimension, random_seed = 8128).to(device)
                        ...
                        new_rel_emb_norm = model.relation_representations[1](indices= None).clone().detach().to(device)
                        ...
                        with torch.no_grad():
                            ...
                            for j in old_rel_to_id:
                                ...
                                old_vector = old_rel_emb_norm[old_idx].to(device)
                                new_rel_emb_norm[new_idx] = old_vector
                            ...
                            model.relation_representations[1]._embeddings.weight.data.copy_(new_rel_emb_norm)
                        ...
                        if initialization == 1: # Initialize new entities with the average of their classes:
                            ...
                            new_ent_emb_norm = init_class_avg(old_ent_to_id,old_ent_emb_norm,new_ent_to_id,new_ent_emb_norm,emb_dimension,pruned_dict,random_noise, sd_frac).to(device)
                            with torch.no_grad():
                                model.entity_representations[0]._embeddings.weight.data.copy_(new_ent_emb)
                                model.entity_representations[1]._embeddings.weight.data.copy_(new_ent_emb_norm)
                        ...
                        old_rel_emb_norm = deepcopy(model.relation_representations[1](indices= None).clone().detach()).to(device)
