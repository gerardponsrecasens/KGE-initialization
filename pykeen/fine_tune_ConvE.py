            ...
            for num_finetune_epochs in nums_finetune_epochs:
                ...
                for s in range(number_snapshots):
                    ...
                    if s == 0:
                        ...
                        # Create the TriplesFactory for the training set with inverse triples
                        training_triples_factory = TriplesFactory.from_path(
                            path=data_path + '0/train.txt',
                            create_inverse_triples=True
                        )

                        # Use the entity and relation mappings from the training factory for testing and validation
                        testing_triples_factory = TriplesFactory.from_path(
                            path=data_path + '0/test.txt',
                            entity_to_id=training_triples_factory.entity_to_id,
                            relation_to_id=training_triples_factory.relation_to_id
                        )

                        validation_triples_factory = TriplesFactory.from_path(
                            path=data_path + '0/valid.txt',
                            entity_to_id=training_triples_factory.entity_to_id,
                            relation_to_id=training_triples_factory.relation_to_id
                        )

                        # Pass the factories to the pipeline
                        train = pipeline(
                            ...
                            training_kwargs={'num_epochs': num_epochs, 'use_tqdm_batch': False, 'drop_last': False},
                        ...
                        old_ent_emb_norm = train.model.entity_representations[1](indices= None).clone().detach().to(device)
                        old_rel_emb = train.model.relation_representations[0](indices= None).clone().detach().to(device)

                        # Get the state_dict from the old model
                        old_state_dict = model.state_dict()

                        # Remove the keys to exclude
                        keys_to_exclude = [
                        ...
                        training_triples_factory = TriplesFactory.from_path('temp/train.txt',create_inverse_triples = True)

                        # Initialize the model with all the entities/relations
                        model = ConvE(triples_factory=training_triples_factory, embedding_dim=emb_dimension, random_seed = 8128).to(device)
                        ...
                        new_ent_emb_norm = model.entity_representations[1](indices= None).clone().detach().to(device)
                        ...
                        with torch.no_grad():
                            for i in old_ent_to_id:
                                ...
                                old_vector = old_ent_emb_norm[old_idx].to(device)
                                new_ent_emb_norm[new_idx] = old_vector
                            ...
                            new_ent_emb_norm = new_ent_emb_norm.unsqueeze(1)
                            model.entity_representations[0]._embeddings.weight.data.copy_(new_ent_emb)
                            model.entity_representations[1]._embeddings.weight.data.copy_(new_ent_emb_norm)
                            model.relation_representations[0]._embeddings.weight.data.copy_(new_rel_emb)
                        ...
                        if early_stopping_finetune:
                            ...
                            tl = training_loop.train(triples_factory=training_triples,drop_last=False, batch_size=min(256,len(training_triples.mapped_triples)),
                        ...
                        old_ent_emb_norm = deepcopy(model.entity_representations[1](indices= None).clone().detach()).to(device)
