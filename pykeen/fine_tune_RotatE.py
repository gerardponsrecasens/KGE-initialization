            ...
            for num_finetune_epochs in nums_finetune_epochs:
                ...
                for s in range(number_snapshots):
                    ...
                    if s == 0:
                        ...
                        model = RotatE(triples_factory=training_triples_factory, embedding_dim=emb_dimension,  random_seed = seed).to(device)
                        # Get new embeddings
                        ...
                        with torch.no_grad():
                            ...
                            # Al operar es generen canvis en com es guarden els vectors
                            
                            # Separate real and imaginary parts
                            real_part = new_ent_emb.real
                            imag_part = new_ent_emb.imag

                            # Determine the shape for the interleaved tensor
                            interleaved_shape = (new_ent_emb.shape[0], new_ent_emb.shape[1] * 2)  

                            # Initialize the interleaved tensor on the correct device
                            interleaved_tensor = torch.empty(interleaved_shape, dtype=torch.float32, device=device)

                            # Interleave real and imaginary parts along the last dimension (columns)
                            interleaved_tensor[:, 0::2] = real_part  # Place real parts in even columns
                            interleaved_tensor[:, 1::2] = imag_part  # Place imaginary parts in odd columns
                            model.entity_representations[0]._embeddings.weight.copy_(interleaved_tensor)



                            # Separate real and imaginary parts
                            real_part = new_rel_emb.real
                            imag_part = new_rel_emb.imag

                            # Determine the shape for the interleaved tensor
                            interleaved_shape = (new_rel_emb.shape[0], new_rel_emb.shape[1] * 2)  
                            ...
                            interleaved_tensor[:, 0::2] = real_part  # Place real parts in even columns
                            interleaved_tensor[:, 1::2] = imag_part  # Place imaginary parts in odd columns
                            model.relation_representations[0]._embeddings.weight.copy_(interleaved_tensor)
                        ...
                        if initialization == 1: # Initialize new entities with the average of their classes:
                            ...
                            new_ent_emb = init_class_avg_RotatE(old_ent_to_id,old_ent_emb,new_ent_to_id,new_ent_emb,emb_dimension,pruned_dict,random_noise, sd_frac).to(device)
                            with torch.no_grad():
                                # Separate real and imaginary parts
                                real_part = new_ent_emb.real
                                imag_part = new_ent_emb.imag
